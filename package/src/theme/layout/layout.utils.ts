...
export function columnSpan({
  ...
  if (mode === 'fixed') {
    return (
      columns * theme.layout.columnWidth + (columns - 1) * theme.layout.columnGap + paddingX * 2
    );
  }
...
export type RowSnapMode = 'marginY' | 'marginTop' | 'marginBottom' | 'padding' | 'expand';
 ...
 */
export function rowSpan({ rem, mode = 'expand', theme }: RowSpanProps): Record<string, string> {
  const snapped = Math.ceil(rem / theme.layout.verticalRhythm) * theme.layout.verticalRhythm;
 ...
 */
export function fullWidth({ theme, startOffset = 0, endOffset = 0, max = true }: LayoutContext): {
  width: string;
  marginLeft: string;
} {
  const totalOffset = startOffset + endOffset;
  const width = max
    ? `min(calc(100vw - ${rem(totalOffset)}), ${theme.layout.maxFullWidth}rem)`
    : `calc(100vw - ${rem(totalOffset)})`;
  return {
    width,
 ...
 */
export function fullHeight({ theme, offset = 0, max = true }: LayoutContext): string {
  return max
    ? `min(calc(100vh - ${rem(offset)}), ${theme.layout.maxFullHeight}rem)`
    : `calc(100vh - ${rem(offset)})`;
}
