 ...
 */
export function createWidthBreakpoints(layout: Layout): Record<MantineBreakpoint, string> {
  const widthBreakpointsValues = {} as Record<MantineBreakpoint, string>;

  widthBreakpointsValues['xl'] = `${layout.maxFullWidth}rem`;

  let currentPaddingX = layout.paddingX.xs;

  for (const breakpoint of Object.keys(DIMENSIONS_RANK) as MantineBreakpoint[]) {
    const paddingX = layout.paddingX[breakpoint] ?? currentPaddingX;
    if (widthBreakpointsValues[breakpoint]) {
      continue;
    }
