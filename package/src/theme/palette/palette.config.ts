import { alpha, lighten, type MantineColorsTuple, type MantineTheme } from '@mantine/core';
...
for (const [color, hue] of Object.entries(HUES)) {
  ...
  COLORS[color] = chroma(hslColor).hex(); // Remove the '#' for hex value
}
...
export const COLOR_SCHEME_PALETTE_RESOLVER = (
  ...
  const baseShade = (scheme: 'light' | 'dark') =>
    typeof theme.primaryShade === 'object' ? theme.primaryShade[scheme] : theme.primaryShade;
  ...
  const getFilledVars = (
    ...
    return {
      ...
      [`--onoko-color-${color}-filled-focus`]:
        scheme === 'dark' ? alpha(shades[offset(base, -2)], 0.3) : shades[offset(base, -2)],
      [`--onoko-color-${color}-filled-touch`]:
        scheme === 'dark' ? alpha(shades[base >= 5 ? 1 : 7], 0.6) : shades[base >= 5 ? 1 : 7],
    };
  };
  ...
  const getOutlineVars = (
    ...
    return {
      [`--mantine-color-${color}-outline`]: scheme === 'light' ? shades[6] : shades[4],
      [`--mantine-color-${color}-outline-hover`]: alpha(shades[scheme === 'light' ? 5 : 4], 0.1),
   ...
   */
  const getWhiteVars = (color: string, shades: MantineColorsTuple): Record<string, string> => {
    return {
