...
export function getContrastColor({
  ...
  const defaultColor =
    theme.colors[theme.primaryColor][
      typeof theme.primaryShade === 'object' ? theme.primaryShade[colorScheme] : theme.primaryShade
    ];
  ...
  if (!_autoContrast) {
    return parsedDefaultColor.isLight ? 'var(--mantine-color-black)' : 'var(--mantine-color-white)';
  }
  ...
  if (variant === 'gradient') {
    ...
    return parsedFrom.isLight ? 'var(--mantine-color-black)' : 'var(--mantine-color-white)';
  }

  return parsedColor.isLight ? 'var(--mantine-color-black)' : 'var(--mantine-color-white)';
}
